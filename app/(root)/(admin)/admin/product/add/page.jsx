...
const AddProductPage = () => {
  ...
  const onAddProductFormSubmit = async (values) => {
    setLoading(true);
    try {
      ...
      const { data: response } = await axios.post("/api/product/add", values);
      if (!response.success) {
    ...
    <div>
      ...
      <Card className="shadow-sm rounded p-0">
        <CardHeader className="border-b px-6 py-2 !pb-1">
          <h2 className="uppercase text-xl font-semibold">Add Product</h2>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onAddProductFormSubmit)}>
