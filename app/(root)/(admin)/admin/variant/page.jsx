...
import { VARIANT_TABLE_COLUMN } from "@/lib/column";
...
const VariantPage = () => {
  const columns = useMemo(() => {
    return columnConfig(VARIANT_TABLE_COLUMN);
  }, []);
  const action = useCallback((row, deleteType, onDelete) => {
    let actionMenu = [];
    actionMenu.push(
      <ActionEdit key="edit" href={ADMIN_VARIANT_EDIT(row.original._id)} />
    );
    actionMenu.push(
      <ActionDelete
        key="delete"
      ...
      <Card className="shadow-sm rounded p-0">
        <CardHeader className="border-b px-6 py-2 !pb-1">
          <div className="flex justify-between items-center">
            <h2 className="uppercase text-xl font-semibold">Variant</h2>
            <Button type="button">
              <Link
                className="flex justify-center items-center gap-1"
                href={ADMIN_VARIANT_ADD}
              >
                <FiPlus />
                Add Variant
              </Link>
        ...
        <CardContent className="px-4">
          <TableWrapper
            ...
            exportEndpoint="/api/variant/export"
            deleteType="SD"
            deleteEndpoint="/api/variant/delete"
            trashView={`${ADMIN_TRASH}?trashof=variant`}
